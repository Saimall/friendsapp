...
app.post("/partner/deny", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error denying partner request." });
  }
});
...
app.get("/partner", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching unverified partners." });
  }
});
...
app.get("/partner/verified", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error fetching verified partners." });
  }
});
...
app.get("/partner/denied", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error fetching denied partners." });
  }
});
...
app.post("/partner/signup", async (req, res) => {
  try {
    const { name, contact, email, password, city, otp } = req.body;
...
app.put(
  "/partner/:partnerId/verify/documents",
  upload.array("images", 2),
  async (req, res) => {
    try {
      const partnerId = req.params.partnerId;
      const { aadharNum, panNum } = req.body;

      if (!aadharNum || !panNum) {
        return res.status(400).json({
          success: false,
          message: "Missing aadharNum or panNum",
        });
      }
      ...
      if (!files || files.length !== 2) {
        return res.status(400).json({
          success: false,
          message: "Please upload exactly two images",
        });
      }
      ...
      partner.documents = imageUrls;
      partner.status = "Unverified";
      await partner.save();

      return res.json({
        success: true,
        message: "Verification successful. Partner documents updated.",
      });
    } catch (err) {
      console.error(err); // Log the actual error for debugging
      return res.status(500).json({
        success: false,
        message: "Error verifying and updating documents.",
      });
    }
  }
);
...
app.post(
  "/partner/:partnerId/verify/documents",
  upload.fields([{ name: "aadhardoc" }, { name: "pandoc" }]),
  async (req, res) => {
    try {
      const partnerId = req.params.partnerId;
      const { aadharNum, panNum } = req.body;
      ...
      const aadharFile = req.files["aadhardoc"][0];
      ...
      const aadhardoc = fileToBase64(aadharFile);
      const pandoc = fileToBase64(panFile);
      ...
      console.log("aadhardoc", partnerData.aadhardoc);
      ...
      partner.aadharNum = partnerData.aadharNum;
      partner.panNum = partnerData.panNum;
      partner.aadhardoc = partnerData.aadhardoc;
      partner.pandoc = partnerData.pandoc;
      partner.status = "Unverified";
      await partner.save();
...
app.get("/partners/:partnerId/city", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Unable to fetch partner service cities." });
  }
});
...
app.put("/partner/:partnerId/status", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error updating partner status." });
  }
});
